...
type Positions struct {
	AvgPx    float64 `json:"avgPx,string"`    //开仓平均价
	PosSide  string  `json:"posSide"`         //持仓方向
	Pos      float64 `json:"pos,string"`      //持仓数量，逐仓自主划转模式下，转入保证金后会产生pos为0的仓位
	AvailPos string  `json:"availPos"`        //可平仓数量，适用于 币币杠杆,交割/永续（开平仓模式），期权（交易账户及保证金账户逐仓）。
	Upl      float64 `json:"upl,string"`      //未实现收益
	UplRatio float64 `json:"uplRatio,string"` //未实现收益率
}
...
type Coin struct {
	...
	CtMult    float64 `json:"ctMult,string"` //合约乘数，仅适用于交割/永续/期权
	...
	Lever     float64 `json:"lever,string"`  //该instId支持的最大杠杆倍数，不适用于币币、期权
	TickSz    float64 `json:"tickSz,string"` //下单价格精度，如 0.0001
	LotSz     float64 `json:"lotSz,string"`  //下单数量精度，如 BTC-USDT-SWAP：1
	MinSz     float64 `json:"minSz,string"`  //最小下单数量
