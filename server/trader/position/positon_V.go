	...
	if this.IsEmpty(){ //空仓
		...
		case common.OperType_Buy:
			if this.Direction() == common.OperType_Buy{  //方向和策略信号一直 加仓位
				...
				percent := int((tick.Buy - this.First_price)/this.First_price * 100) //涨幅
				need_add := int((danci_add * percent  + int(this.First_num) - int(this.Num())/danci_add))  //需要加仓的次数
		...
		case common.OperType_Sell:
			if this.Direction() == common.OperType_Sell{  //方向和策略信号一直 加仓位
				...
				danci_add := int(this.First_num * this.Percent_add)  //单次加的数量
				percent := int((this.First_price - tick.Buy)/tick.Buy) //跌幅
				need_add := int((danci_add * percent  + int(this.First_num) - int(this.Num())/danci_add))  //需要加仓的次数
